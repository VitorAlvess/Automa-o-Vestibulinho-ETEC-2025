...
pdf_folder_path = "./resultados"  # Substitua pelo caminho correto
...
def processar_pdfs_na_pasta(pdf_folder_path):
    for arquivo_pdf in os.listdir(pdf_folder_path):
        if arquivo_pdf.endswith(".pdf"):  # Verifica se o arquivo é um PDF
            pdf_path = os.path.join(pdf_folder_path, arquivo_pdf)
            
            # Executar a função para extrair dados do PDF
            ...
            print(f"Planilha salva para o PDF '{arquivo_pdf}' com o nome: {nome_arquivo}")

# Chama a função para processar todos os PDFs na pasta
processar_pdfs_na_pasta(pdf_folder_path)
